...
class CheckOutService:
    @classmethod
    @transaction.atomic
    def check_out(cls, *, company: Company, membership: Membership, method: str, evidence: dict, actor: Membership) -> AttendanceEvent:
        current_status = LiveAttendanceService.get_member_status(company=company, membership=membership)
        AttendanceEventValidator.validate_workflow_state_transition(current_status, AttendanceEventTypes.CHECK_OUT)
